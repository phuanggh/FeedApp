...
//  FeediOSTests
//
//  Created by Penny Huang on 2023/2/13.
//
import XCTest
import UIKit
import Feed
...
// Temporal Coupling: require certain methods of a class to be called in a specific order
final class FeedUIIntegrationTests: XCTestCase {
    // 測試FeedViewController上，UI元件顯示與否、UI觸發的action是否如期執行
    // This test tests the whole composition of UI and the Presentation -> integration test
    ...
    func test_viewDidLoad_showsLoadingIndicator() {
        ...
        sut.loadViewIfNeeded()
        // loading indicator shows when it is refreshing
    ...
    func test_loadFeedCompletion_rendersSuccessfullyLoadedFeed() {
        ...
        // to abstract the test from concrete UI elements (e.g. TableView)
        // The goal is to test the behaviour, not the implementation
        // hide the implementation of TableView from the test
        // so that we can change it to CollectionView or others without breaking the tests
        
        // Test no cell is rendered before loading completes
        XCTAssertEqual(sut.numberOfRenderedFeedImageViews(), 0)
        
        // Test number of cell rendered
    ...
    func test_feedImageViewRetryButton_isVisibleOnImageURLLoadError() {
        ...
        XCTAssertEqual(view1?.isShowingRetryAction, false, "Expected no retry action for first view while loading second image")
    ...
    func test_feedImageViewRetryButton_isVisibleOnInvalidImageData() {
        ...
        XCTAssertEqual(view?.isShowingRetryAction, false, "Expected no retry action while loading iamge")
        
        let invalidImageData = Data("invalid image data".utf8)
        loader.completeImageLoading(with: invalidImageData, at: 0)
        XCTAssertEqual(view?.isShowingRetryAction, true, "Expected retry action while image loading completew with invalid image data")
    }
    ...
    func test_feedImageViewRetryAction_retriesImageLoad() {
        ...
        XCTAssertEqual(loader.loadedImageURLs, [image0.url, image1.url, image0.url, image1.url], "Expected fourth image URL requests after secons view retry action")
    }
    ...
    func test_feedImageView_cancelsImageURLPreloadingWhenNotNearVisibleAnymore() {
        ...
        XCTAssertEqual(loader.loadedImageURLs, [image0.url], "Expected first cancelled image URL requests once first image is not near visible anymore")
        
        sut.simulateFeedImageViewNotNearVisible(at: 1)
        XCTAssertEqual(loader.loadedImageURLs, [image0.url, image1.url], "Expected second cancelled image URL requests once second image is not near visible anymore")
    }
    ...
    func test_loadImageFeedCompletion_dispatchesFromBackgroundToMainThread() {
        let (sut, loader) = makeSUT()
        sut.loadViewIfNeeded()
        
        loader.completeFeedLoading(with: [makeImage()])
        _ = sut.simulateFeedImageViewNotVisible(at: 0)
